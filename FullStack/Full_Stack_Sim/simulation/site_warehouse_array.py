...
class ForkliftSprite(pygame.sprite.Sprite):
    ...
    def _create_surface(self, facing_right):
        # Canvas size accommodates the tall mast
        surf = pygame.Surface((self.width + 50, self.height + self.max_fork_height + 20), pygame.SRCALPHA)
        
        # Coordinate Reference: (0,0) is top-left of surface. 
        # We draw relative to the "floor" at the bottom.
        floor_y = surf.get_height() - 10
        
        # Directional multipliers
        direction = 1 if facing_right else -1
        # If facing right, body starts at 0. If left, body starts at width.
        ...
        for tx in [rear_tire_x, front_tire_x]:
            pygame.draw.circle(surf, (20, 20, 20), (tx, floor_y), tire_radius) # Rubber
            pygame.draw.circle(surf, (50, 50, 50), (tx, floor_y), 6) # Hubcap
            # Treads
    ...
    def update(self):
        ...
        if self.state in ['LIFTING', 'LOWERING']:
            ...
            if abs(dh) < 1:
                ...
                # Re-render to update fork position
        ...
        if self.cargo_sprite:
            ...
            # Center on forks (fork width 35, crate width 40)
    ...
    def draw(self):
        ...
        for sprite in self.all_sprites:
            if isinstance(sprite, ForkliftSprite):
                ...
                # Shadow only when on floor (forklift carries it otherwise)
