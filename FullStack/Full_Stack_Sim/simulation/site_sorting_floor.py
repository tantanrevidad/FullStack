...
# --- Visualization Layer (Overhauled) --------------------------------------
...
class SortingSimulation:
    ...
    def _init_sort_menu(self):
        menu_btn_w, menu_btn_h = 220, 40; menu_cx = self.SIM_WIDTH // 2; start_y = 200
        ...
        for r in range(rows):
            ...
            for c in range(cols):
                ...
                if random.random() > 0.2:
                    box_w = random.randint(10, int(w/cols - 5)); box_h = random.randint(5, int(h/rows - 5))
                    box_x = cx + random.randint(2, int(w/cols - box_w - 2)); box_y = ry + int(h/rows - box_h - 2)
    ...
    def _generate_static_background(self):
        ...
        self._draw_shelf_unit(bg, 0, 100, SCREEN_WIDTH, SCREEN_HEIGHT/2, 8, 25) # Increased cols
        
        # Extend pillars across the full screen
        for i in range(7):
        ...
        for _ in range(8000): # Increased noise density
        ...
        for i in range(0, SCREEN_WIDTH, 150):
            pygame.draw.rect(bg, (20,22,25), (i, 30, 100, 10)); pygame.draw.rect(bg, FLUORESCENT_LIGHT, (i+2, 32, 96, 6))
            light_fixtures.append((i+50, 40))
            
        rail_y = self.gantry_crane.y_pos - 30
        pygame.draw.rect(bg, (50, 55, 60), (0, rail_y, SCREEN_WIDTH, 25)); pygame.draw.rect(bg, (80, 85, 90), (0, rail_y, SCREEN_WIDTH, 25), 2)
    ...
    def action_load_containers(self):
        ...
        try:
            size = int(size_text)
            if not 10 <= size <= 30: self.lcd.update_status("ERR: SIZE 10-30"); return
        except ValueError: self.lcd.update_status("ERR: NOT A NUMBER"); return
        self.lcd.update_status("LOADING CARGO...")
        self.logic.generate_new_list(size=size)
        for c in self.containers: c.kill()
        ...
        for i, value in enumerate(self.logic.items):
            ...
            self.containers.add(c); self.all_sprites.add(c); self.visual_containers.append(c)
    ...
    def action_begin_sort(self, sort_type):
        ...
        st_map = {"bubble": self.logic.get_bubble_sort_steps, "selection": self.logic.get_selection_sort_steps,
                  "insertion": self.logic.get_insertion_sort_steps, "merge": self.logic.get_merge_sort_steps,
    ...
    def action_reset(self):
        for sprite in self.all_sprites:
            sprite.is_moving = False; sprite.on_finish_callback = None
        self.is_sorting = False; self.sort_steps.clear(); self.show_sort_menu = False
        self.lcd.update_status("SYSTEM RESET"); self.action_load_containers()

    def increase_speed(self):
        if self.speed_index < len(self.speed_levels) - 1:
            self.speed_index += 1; self.speed_multiplier = self.speed_levels[self.speed_index]
    ...
    def reset_container_states(self):
        for c in self.visual_containers:
            if c.state != 'sorted': c.set_state('idle')
    ...
    def process_next_step(self):
        if not self.sort_steps or not self.is_sorting:
            self.is_sorting = False; self.lcd.update_status("SORT COMPLETE"); self.reset_container_states(); return
        step = self.sort_steps.pop(0)
        self.reset_container_states()
        step_type = step.get("type")
        if step_type == "compare":
            idx1, idx2 = step["indices"]; c1, c2 = self.visual_containers[idx1], self.visual_containers[idx2]
            c1.set_state('comparing'); c2.set_state('comparing')
            ...
            pos1_x, pos2_x = self.slot_positions[idx1], self.slot_positions[idx2]
            c1.move_to((pos2_x, c1.pos_y)); c2.move_to((pos1_x, c2.pos_y), self.process_next_step)
        elif step_type in ["lock", "set_pivot", "highlight"]:
            state = {"lock": "sorted", "set_pivot": "pivot", "highlight": "highlight"}[step_type]
            for idx in step.get("indices", [step.get("index")]):
                if 0 <= idx < len(self.visual_containers): self.visual_containers[idx].set_state(state)
            self.process_next_step()
        elif step_type == "reorder":
            indices, values = step["indices"], step["values"]
            value_map = {c.value: c for c in self.visual_containers}
            ...
            for c in self.visual_containers: c.set_state('sorted')
            self.process_next_step()

    def handle_events(self, event):
        if self.show_sort_menu:
            for btn in self.sort_menu_buttons: btn.handle_event(event)
            if event.type == pygame.MOUSEBUTTONDOWN:
                if not any(btn.rect.collidepoint(event.pos) for btn in self.sort_menu_buttons):
                    self.show_sort_menu = False
            return
        self.lcd.handle_event(event); self.btn_reset.handle_event(event)
        self.btn_speed_up.handle_event(event); self.btn_speed_down.handle_event(event)
        if not self.is_sorting:
            self.btn_load.handle_event(event); self.btn_sort.handle_event(event)
    ...
    def draw_sort_menu(self):
        ...
        for btn in self.sort_menu_buttons: btn.draw(self.screen)
    ...
    def draw(self):
        ...
        for container in self.containers:
            ...
            label_surf = pygame.Surface(label_size); label_surf.fill(WHITE)
        ...
        self.btn_load.draw(self.screen); self.btn_sort.draw(self.screen); self.btn_reset.draw(self.screen)
        self.btn_speed_up.draw(self.screen); self.btn_speed_down.draw(self.screen)
        if self.show_sort_menu: self.draw_sort_menu()